...
#define c4_real(id)                   c4_sysux(C4_REAL, MKWORD(0, id), NULL, NULL)
#define c4_ipcx(type, id, to, sd, rv) c4_sysux(type, MKWORD(to, id), sd, rv)
...
static inline
uint c4_ipcxw(uint type, uchar id, uchar to, uint w)
{
	UT_REGS reg;
	reg.dx = w;
	c4_ipcx(type, id, to, &reg, &reg);
	return reg.dx;
}
#define c4_ipcw(to, w)     c4_ipcxw(C4_IPC, C4_NIL, to, w)
#define c4_ipc(to, sd, rv) c4_ipcx (C4_IPC, C4_NIL, to, sd, rv)
