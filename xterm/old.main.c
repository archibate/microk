...
static int xwm_init(void)
{
	int i = fopen_s(&fxwm, "/dev/xwmc", "r");
	if (!fxwm)
		return i;
	return 0;
}
...
static int xwm_open_video(VIDEO_INFO *video)
{
	void *winbuf = (void *) 0x80000000;
	int i, size;

	i = open("/tmp/win1info", OPEN_RD);
	if (i < 0)
		return i;
	size = read(i, video, sizeof(VIDEO_INFO));
	close(i);

	i = open("/tmp/win1", OPEN_RD | OPEN_WR);
	if (i < 0)
		return i;
	size = tellsize(i);
...
static void xwm_quit(void)
{
	fclose(fxwm);
	fxwm = 0;
}

static void xwm_flush(void)
{
	fputc('\n', fxwm);
}

static int xwm_invalidate_rect(int x, int y, int xs, int ys)
{
	fprintf(fxwm, "U%d:%d:%d:%d;", x, y);
	return 0;
}
...
int main(void)
{
	...
	xwm_init();
	xwm_open_video(&video);

	i = fopen_s(&fin, "/dev/vmon0", "r");
	if (!fin)
